...
/**
 * @ Modified time: 2020-03-17 22:25:36
 * 
 */
/**
 * Options page entity.
...
class antibot_Page {
	...
	public function set_markup_top( $markup = false )
	{
        ...
        $this->markup_top .=  '<img id="bill_admin_logo" src="'.ANTIBOTSURL.'/assets/images/logo.png" />';
 		} else {
...
class antibot_Tab {
	...
	public function __construct ( $title, $id, $page, $section_option_settings = array(), $active_state = FALSE ) 
	{
		...
		if($this->active) {
			new antibot_SectionFactory( $page, $section_option_settings, $this );
		}
	}
	public function __get($key)
	{
...
class antibot_Section {
	...
	public function __construct ( $id, $title, $info, $page, $page_key, $field_settings = array() ) 
	{
		...
		$this->settings_factory = new antibot_SettingsFactory( $this, $field_settings );
	}
	public function __get($key)
	{
		if(property_exists($this, $key)) {
