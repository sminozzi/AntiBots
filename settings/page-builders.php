<?php  namespace AntibotsWPSettings;
	...
	public function register_page()
	{
		...
		switch($this->page->type) {
			...
			case 'menu':
 			...
 			case 'submenu2':        
                   add_submenu_page(
                   'anti_bots_plugin', // parent slug
                   'Settings', // title
                   'Settings1', // menu title
                   $this->page->capability, 
                   $this->page->slug, // menu slug anti-bots
                   //'antibots_settings', // menu slug 
                    array($this, 'render'));
                break;
			case 'submenu':
				add_submenu_page( 
		...
		if($page_key == $page->slug && $action == 'update') {
			...
			foreach( $options_settings as $title=>$section_settings ) {	
				...
				if(isset($matches[1]) && $matches[1] == $id) {
					...
					$this->antibot_tabs[] = new antibot_Tab( $title, $id, $this->page, $section_settings, TRUE );
					break;
				}
			...
			if(empty($this->tabs)) {
				$this->tabs[] = new antibot_Tab( $first['title'], $first['id'], $this->page, $first['settings'], TRUE );
			}
		} else {
			...
			foreach( $options_settings as $title=>$section_settings ) {	
				...
				if($counter == 0) {
					$this->tabs[] = new antibot_Tab( $title, $id, $this->page, $section_settings, TRUE );
				} else {
					$this->tabs[] = new antibot_Tab( $title, $id, $this->page, $section_settings );
				}
				$counter++;
			}
		}
